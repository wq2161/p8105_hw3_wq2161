...
In 2002, there were `r nrow(state_2002)` states which were observed at 7 or more locations. In 2010, there were `r nrow(state_2010)` states which were observed at 7 or more locations.
...
accel_data = accel_data %>%
  ...
  mutate(
    ...
    activity_min = as.character(activity_min),
...
The dataset is a record of 5 weeks of accelerometer data collected on a 63 year-old male with BMI 25, who was admitted to the Advanced Cardiac Care Center of Columbia University Medical Center and diagnosed with congestive heart failure (CHF). The dataset includes `r ncol(accel_data)` variables, which are `r colnames(accel_data)`, and `r nrow(accel_data)` observations. Specifically, the activity_min is the activity counts for each minute of a 24-hour day starting at midnight.
